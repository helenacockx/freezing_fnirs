...
rec_nirs=[1];
nirs_offline=false;
rec_motion=[1 1 1 1 1 1];
take_motion=[1 2 3 4 5 6]; %of all recordings
run_motion=[3 4 5 6 7 8];
rec_stim=[1 2 3];
...
if success
    ...
    if length(files)-2~=8
      warning('n\ not all 8 screenshots were captured')
    end
end
...
if length(files)==1
    ...
    warning('\n No file named *.bodydimensions.mvna or multiple files named like that');
end

% move motion files to source directory
for i=21:length(rec_motion)
    for e=1:length(ext)
